  ...
  cow_surv_times= simple.surv.sim(n= NumCows, 
                                  foltime=end_time, 
                                  dist.ev = c("weibull"), 
                                  anc.ev=p_cow,beta0.ev=B_0_cow, 
                                  dist.cens = c("unif"), 
                                  anc.cens=end_time, 
                                  beta0.cens=end_time)
  ...
  for (t in 1:length(times)){
    if (t>1){
      NumCowsDie = length(which(cow_surv_times$stop >= Pop_t$times[t-1] &  ### $times??
                                cow_surv_times$stop < Pop_t$times[t]))
    ...
    if (Pop_t$NumBorn[t] > 0){ 
      calf_surv_times= simple.surv.sim(n= Pop_t$NumBorn[t], 
                                       foltime=(end_time-t) , 
                                       dist.ev = c("weibull"), 
                                       anc.ev=p_calf,
                                       beta0.ev=B_0_calf, 
                                       dist.cens = c("unif"), 
                                       anc.cens=(end_time-t), 
                                       beta0.cens=(end_time-t))
      # calf_surv_times
      calf_obs_StartTimes = t+round(rnorm(n = Pop_t$NumBorn[t], 
                                          mean = CalfMeanAvail, 
                                          sd = CalfStdAvail))
